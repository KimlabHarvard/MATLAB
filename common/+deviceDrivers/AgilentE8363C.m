classdef AgilentE8363C < deviceDrivers.lib.GPIBorEthernet
    ...
    methods (Access = public)
        ...
        function [frequencies, trace] = getTrace(obj)
            % select measurement
            ...
            span = obj.sweep_span;
            frequencies = linspace(center_freq - span/2, center_freq + span/2, length(trace));
        end
        
        function traces = getAllTraces(obj)
            % select measurement
            % get measurement name
            measurement = obj.measurements;
            % take the part before the comma
            commaPos = [1,strfind(measurement, ',')];
            n_meas = length(commaPos)/2;
            n_points = obj.sweep_points;
            traces = zeros(n_points, n_meas+1);
            traces(:,1) = obj.getX;
            for i=1:n_meas
        ...
        function Xdata = getX(obj)
            ...
            Xdata(1) = str2double(Xstr(1:p(1)));
            for i=2:length(p)
                Xdata(i) = str2double(Xstr(p(i-1):p(i)));
            end
            Xdata(end) = str2double(Xstr(p(end):end));
        end
