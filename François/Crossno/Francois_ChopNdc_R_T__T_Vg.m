...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function data = Francois_ChopNdc_R_T__T_Vg(T_list, Vg_list, Vg_limit, Vg_rampRate,...
    ...
    function plot2Dnoise()
        ...
        surf(data.Vg,data.T,data.VNdc_X');
        xlabel('Vg (Volts)'); ylabel('Temperature (K)');
        view(2);shading flat; colorbar; box on; colormap(flipud(cmap));
    end
    %measures the data
    function measure_data(T_n,Vg_n)
        ...
        while n <= Nmeasurements
            ...
            [data.raw.VNdc_X(T_n,Vg_n,n), data.raw.VNdc_Y(T_n,Vg_n,n)] = Ndc.snapXY();
            
            
            %check if we are between 5% and 95% of the range, if not autoSens
            SD_high = max(abs(data.raw.Vsd_X(T_n,Vg_n,n)),abs(data.raw.Vsd_Y(T_n,Vg_n,n)));
            Ndc_high = max(abs(data.raw.VNdc_X(T_n,Vg_n,n)),abs(data.raw.VNdc_Y(T_n,Vg_n,n)));
            OK = 1;
            ...
            if Ndc_high > Ndc_sens*0.95 || Ndc_high < Ndc_sens*0.05
                Ndc.autoSens(0.25,0.75);
                Ndc_sens = Ndc.sens();
                OK = 0;
            end
            if OK == 1
        ...
        data.VNdc_X(T_n,Vg_n) = mean(data.raw.VNdc_X(T_n,Vg_n,:));
        data.VNdc_Y(T_n,Vg_n) = mean(data.raw.VNdc_Y(T_n,Vg_n,:));
        ...
        data.T(T_n,Vg_n) = mean(data.raw.T(T_n,Vg_n,:));
        data.std.VNdc_X(T_n,Vg_n) = std(data.raw.VNdc_X(T_n,Vg_n,:));
        data.std.VNdc_Y(T_n,Vg_n) = std(data.raw.VNdc_Y(T_n,Vg_n,:));
    end
    ...
    data.raw.VNdc_X = blank_raw;
    data.raw.VNdc_Y = blank_raw;
    ...
    data.settings.VNdc.sinePhase = Ndc.sinePhase;
